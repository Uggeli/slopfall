...
namespace DaggerfallWorkshop.Sim.Engine
{
    ...
    public static class EngineSoak
    {
        ...
        static void Report(SimWorld w, long tick)
        {
            ...
            long cats = 0, recs = 0, placeRecs = 0, placeAtoms = 0, innateAtoms = 0, provAtoms = 0, dangerRecs = 0;
            int memAgents = 0, learned = 0, nodes = 0, reinforced = 0;
            ...
            foreach (var kv in w.AgentMemory.All)
            {
                ...
                // PLACES learning, by salience tier: kind atoms (INNATE) should persist, provisions
                // (ordinary) + danger (SURPRISE) fade at their own rates.
                var places = kv.Value.Stores.Places;
                placeRecs += places.Count;
                for (int pi = 0; pi < places.Count; pi++)
                {
                    var prec = places[pi];
                    placeAtoms += prec.DeltaBag.Count;
                    for (int mk = 0; mk < prec.Meta.Length; mk++) if (prec.Meta[mk].IsInnate) innateAtoms++;
                    if (prec.DeltaBag.TryGet(Memory.PlaceAtoms.Provisions, out _)) provAtoms++;
                    if (prec.DeltaBag.TryGet(Memory.PlaceAtoms.Danger, out var dv))
            ...
            if (memAgents > 0)
            {
                ...
                Console.WriteLine($"           places[rec/agent={(double)placeRecs / memAgents:F1} atom/agent={(double)placeAtoms / memAgents:F1} innate={innateAtoms} prov={provAtoms} dangerRecs={dangerRecs} meanDanger={(dangerRecs > 0 ? dangerSum / dangerRecs : 0):F3}]");
            }
        }
    }
}
